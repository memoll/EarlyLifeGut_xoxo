...
directory <- "~/Documents/xoxo_article/xoxo_qpcr_FINAL/18s"
...
# 
# Remove rows with "_2" or "_5" in the "Sample Name" column
...
colnames(cleaned_results) <- c("SAMPLE_ID", "euk_QUANTITY_MEAN")
...
# Import metadata
meta <- read.delim2("~/Documents/xoxo_article/files/metadata_xoxo_mi_18months_full.csv", header = T, sep = ",")
head(meta);dim(meta)
...
sum(is.na(meta_qpcr$euk_QUANTITY_MEAN)) 

#check for the differences between sample IDs in both dataframes
sum(is.na(meta_qpcr$euk_QUANTITY_MEAN)) - (length(meta_babies$SAMPLE_ID) - length(qpcr_data$SAMPLE_ID))
setdiff(qpcr_data$SAMPLE_ID, meta_babies$SAMPLE_ID) #"XC04-19" "XC05-18"
...
#change XC05-19 to XC05-18 in meta_babies$SAMPLE_ID
which(meta_babies$SAMPLE_ID == "XC05-19")
meta_babies[52,"SAMPLE_ID"]
meta_babies[52,"SAMPLE_ID"] <- "XC05-18"

# merge again
meta_qpcr_cor <- merge(meta_babies, qpcr_data_cor, by = "SAMPLE_ID", all.x = TRUE); dim(meta_qpcr_cor)
head(meta_qpcr_cor)

sum(is.na(meta_qpcr_cor$euk_QUANTITY_MEAN)) 
sum(is.na(meta_qpcr_cor$euk_QUANTITY_MEAN)) - (length(meta_babies$SAMPLE_ID) - length(qpcr_data_cor$SAMPLE_ID))

# Write the data frame to a CSV file
write.csv(meta_qpcr_cor, file = "~/Documents/xoxo_article/files/metadata_xoxo_mi_18months_qpcr_euk.csv", row.names = FALSE)
